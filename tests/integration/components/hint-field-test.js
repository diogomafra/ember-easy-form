...
test('uses the wrapper config', function(assert) {
  ...
  this.render(hbs`{{#form-for model wrapper="my_wrapper"}}{{hint-field firstName text="Some text"}}{{/form-for}}`);

  assert.ok(this.$().find('span.my-hint').get(0), 'hintClass not defined');
});

test('uses the defined template name', function(assert) {
  this.container.register('template:custom-hint-template', hbs`My custom hint | {{hintText}}`);
  config.registerWrapper('my_wrapper', {hintTemplate: 'custom-hint-template'});

  this.render(hbs`{{#form-for model wrapper="my_wrapper"}}{{hint-field firstName text="My text"}}{{/form-for}}`);
