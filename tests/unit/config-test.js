...
test('register custom input types', function(assert) {
  config.registerInputType('my_input', Ember.View);
  config.registerInputType('another_input', Ember.Component);

  var inputType = config.getInputType('my_input');
  assert.equal(inputType, Ember.View);
  inputType = config.getInputType('another_input');
  assert.equal(inputType, Ember.Component);
});
